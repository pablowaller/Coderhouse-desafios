...
class ProductMySqlDAO extends IProductDAO{
    ...
    async getById(id){
        ...
        return new this.DTO( entity[0].id, entity[0].title, entity[0].price, entity[0].thumbnail).toJson()
    }

    async getAll(){
        const data = await knex.from(this.db).select('*')
        return data.map(entity => new this.DTO(entity.id, entity.title, entity.price, entity.thumbnail).toJson() )
    }
